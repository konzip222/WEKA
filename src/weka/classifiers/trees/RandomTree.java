...
public class RandomTree extends Classifier implements OptionHandler,
  ...
  protected ArrayList<Integer> attrs = null;
  ...
  public double splitPoint = -1;
  
  public Random random_val;
  
  public ArrayList<Integer> selected_rules = null;
  
  public int m_NumFolds = 0;
  
  public int percent_data = 100;
  
  public Random percent_data_rand = null;
...
//      m_KValue = data.numAttributes() - 1;
    	m_KValue =  (int) 0.01 * m_KValue * (data.numAttributes() - 1);
    if (m_KValue < 1)
    ...
    Random rand = null;
    rand = data.getRandomNumberGenerator(m_randomSeed);
	...
	for(int i=0 ; i < attIndicesWindow.length; i++) {
		attrs.add(attIndicesWindow[i]);
	}
	
	if (selected_rules != null) {
		attrs.removeAll(selected_rules);
	}
    
	int[] cp_attIndicesWindow = new int[attrs.size()];
	for(int i = 0; i < attrs.size(); i++) cp_attIndicesWindow[i] = attrs.get(i);
	
    if (random_val != null) 
    	rand = random_val;
			
    m_Tree.buildTree(train, classProbs, cp_attIndicesWindow, rand, 0);
...
//    public int getRandomWithExclusion(Random random, int end) {
//    	int rand = end - 1;
//    	rand = random.nextInt(end);
//    	if (end != 1) {
//        	do {
//        	    rand = random.nextInt(end);
//        	} while (attrs.contains(rand) == false);
//    	} 
//    	return rand;
//    }
...
//    	  chosenIndex = getRandomWithExclusion(random, windowSize);
    	  chosenIndex = random.nextInt(windowSize);
