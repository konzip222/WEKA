...
public class SimpleCart
  ...
  private LinkedHashMap<RandomTree, Integer> listRTrees = new LinkedHashMap<RandomTree, Integer>();
...
//---------------------------------------------------------------------------------
      ...
      rfTree.setMaxDepth(1);
      
      rfTree.setNumTrees(3);

      rfTree.buildClassifier(data);
     
	  
      int indexAttr = m_Attribute.index();
...
//---------------------------------------------------------------------------------
  ...
  private DecisionRule getBestDecisionRule(LinkedHashMap<RandomTree, 
		  Integer> listRTrees )
  {
	  int maxVal = 0 ;
	  int i = 0;
	  RandomTree bestTree = null;
	     
	  for (Map.Entry<RandomTree, Integer> entry : listRTrees.entrySet()) 
	  {
		  int value = (int)entry.getValue();
	    	 
		  RandomTree key = (RandomTree)entry.getKey();

		  
/*		  System.out.print("Drzewo o indexie: " + i 
				  + " poprawnie zaklasyfikowalo: " + value + " ");
    	 
		  if(decisionRule.isNominal())
			  System.out.println(attr + " = " + decisionRule.getSplitString());

		  else
			  System.out.println(attr + " = " + decisionRule.getSplitPoint());*/
...
/*		  System.out.print("Najlepszym klasyfikatorem jest drzewo z regula: ");
