 ...
 */
public class myRandomTreeV3 extends Classifier implements OptionHandler,
	...
	public String toString() {
		...
		if (m_Tree == null) {
			return "myRandomTreeV3: no model has been built yet.";
		} else {
			return "\nmyRandomTreeV3\n==========\n"
					+ m_Tree.toString(0)
					+ "\n"
	...
	public String graph() throws Exception {
		...
		if (m_Tree == null) {
			throw new Exception("myRandomTreeV3: No model built yet.");
		}
		StringBuffer resultBuff = new StringBuffer();
		m_Tree.toGraph(resultBuff, 0, null);
		String result = "digraph myRandomTreeV3 {\n" + "edge [style=bold]\n"
				+ resultBuff.toString() + "\n}\n";
		return result;
	}
	...
	protected class Tree implements Serializable {
		...
		public double[] distributionForInstance(Instance instance)
			...
			if (m_Attribute != null) {
				double inst_class = forest.classifyInstance(instance);
				if (inst_class == 0.0) {
		...
		protected String toString(int level) {
			...
			try {
				...
				e.printStackTrace();
				return "myRandomTreeV3: tree can't be printed";
			}
		}
		...
		protected void buildTree(Instances data, double[] classProbs,
			...
			forest.setMaxDepth(1);
			forest.setNumTrees(4);
			forest.buildClassifier(data);
				
			bestDists = getDist(data);
			m_Prop = getProps(bestDists);
			m_Attribute = forest.getAttributes();
			m_SplitPoint = forest.getSplitPoints();
				
			if (Arrays.equals(m_ClassDistribution, bestDists[1]) || m_Attribute.length == 0 || 
	    ...
	    public double [][] getDist(Instances data) throws Exception {
	    	...
	    	for (int i = 0; i < data.numInstances(); i++) {
	            Instance inst = data.instance(i);
	            double inst_class = forest.classifyInstance(inst);
	            if (inst_class == 0.0) {
		...
		protected Instances[] splitData(Instances data) throws Exception {
			...
			for (int i = 0; i < data.numInstances(); i++) {
				...
				double inst_class = -1;
				inst_class = forest.classifyInstance(inst);
	...
	public static void main(String[] argv) {
		runClassifier(new myRandomTreeV3(), argv);
	}
}
