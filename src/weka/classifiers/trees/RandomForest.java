...
public class RandomForest extends Classifier implements OptionHandler,
  ...
  private double [] splitPoints = null;
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    m_bagger.percent_data_rand = percent_data_rand;
    ...
    List<RandomTree> trees = m_bagger.getRTrees();
    attributes = new int [trees.size()];
    splitPoints = new double [trees.size()];

    int index = 0;
    
    for (RandomTree tree: trees) {
    	if (tree.attribute != -1) {
    		attributes[index] = tree.attribute;
    		splitPoints[index] = tree.splitPoint;
    		index++;
    	}
    }    
  }
